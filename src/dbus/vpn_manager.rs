...
impl VpnManager {
    pub(crate) fn new(conn: &zbus::Connection) -> Self {
    ...
    pub(crate) async fn list_profiles(&self) -> zbus::Result<Vec<VpnProfile>> {
        ...
        for conn_path in connections {
            ...
            profiles.push(VpnProfile { name, uuid, connection_path: conn_path.to_string() });
        }
        ...
        profiles.sort_by(|a, b| {
            a.name
                ...
                .then(a.connection_path.cmp(&b.connection_path))
        });
        Ok(profiles)
    }

    /// Return active VPN connections keyed by Settings.Connection path.
    pub(crate) async fn active_by_connection_path(
        &self,
    ) -> zbus::Result<HashMap<String, VpnActive>> {
        let nm = NetworkManagerProxy::new(&self.conn).await?;
        // An unavailable D-Bus property is an error, not an empty VPN list.
        // Treating it as empty makes a transient read failure look like every
        // VPN was disconnected and causes the UI to render a false state.
        let actives = nm.active_connections().await?;
        ...
        for active_path in actives {
            ...
            let conn_type = active.connection_type().await?;
            if conn_type != "vpn" && conn_type != "wireguard" {
                continue;
            }
            ...
            out.insert(
                ...
                VpnActive { active_path: active_path.to_string(), state, connection_path },
            );
        }
    ...
    pub(crate) async fn connect(&self, connection_path: &str) -> zbus::Result<OwnedObjectPath> {
        let nm = NetworkManagerProxy::new(&self.conn).await?;
    ...
    pub(crate) async fn disconnect(&self, active_path: &str) -> zbus::Result<()> {
        let nm = NetworkManagerProxy::new(&self.conn).await?;
    ...
    pub(crate) async fn delete_profile(&self, connection_path: &str) -> zbus::Result<()> {
        let conn_path = ObjectPath::try_from(connection_path)
            .map_err(|e| zbus::Error::Failure(format!("Invalid connection path: {e}")))?;
        let conn = SettingsConnectionProxy::builder(&self.conn).path(conn_path)?.build().await?;
        conn.delete().await
    }
}
