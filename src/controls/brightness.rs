...
pub(crate) struct BrightnessManager {
...
impl BrightnessManager {
    ...
    pub(crate) const MIN_BRIGHTNESS_PERCENT: f64 = 1.0;

    /// Creates a new BrightnessManager. Discovers the hardware path dynamically.
    pub(crate) async fn new() -> ZbusResult<Self> {
        ...
        if let Ok(entries) = fs::read_dir("/sys/class/backlight") {
            let mut devices: Vec<_> = entries.flatten().filter(|e| e.path().is_dir()).collect();
            ...
            if let Some(entry) = devices.into_iter().next() {
                ...
                if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
                    ...
                    backlight = Some(BacklightInfo { dir: path, name: name_str });
                }
            }
        }
    ...
    pub(crate) fn get_brightness_percent(&self) -> Option<f64> {
    ...
    pub(crate) async fn set_brightness_percent(
        &self,
        percent: f64,
    ...
    pub(crate) fn watch_changes<F>(self: &Rc<Self>, interval_ms: u32, callback: F) -> glib::SourceId
    where
        F: Fn(f64) + 'static,
    {
